...
|---|:---:|:---:|:---:|
| `queueMicrotask` | no | ~1 µs | No -- drains entire queue per tick |
| `setTimeout(fn, 0)` | no | ~1 ms minimum (clamp) | No -- coarse, single-threaded queue |
| `requestAnimationFrame` | no | tied to display refresh | Yes, but tied to paint timing |
...
| Chrome / Edge 32+ | yes |
| Firefox 41+ | yes |
| Safari 12+ (iOS 12+) | yes |
| Node.js 18+ | yes |
| Bun / Deno | yes |
| Twitch Extension iframe | yes (well under 1 MB / 3 s budget) |
